...
from tmtc_quan_filters import TMTc_filterByIntensity
from outliers import outlierRemoval


def TMTc_summarization_1_2(df, params):
    # Summarization of proteins (and peptides?) mapped by 1 or 2 PSMs
    # PSM filtering methods and thresholds
    methods = params["tmtc_min_intensity_method_1_2_psm"].split(",")
    thresholds = params["tmtc_min_intensity_value_1_2_psm"].split(",")
    #reporters = params["tmt_reporters_used"].split(";")
    idx = []
    if len(df) == 1:
        # Proteins/peptides mapped by only one PSM
        # If the PSM is filtered by the intensity-based filter, it will not be used for the quantification
        idx = TMTc_filterByIntensity(df, methods, thresholds, 0)
        if len(idx) > 0:
            df.drop(idx, inplace=True)
    elif len(df) == 2:
        # Proteins/peptides mapped by two PSMs
        ...
        if len(idx) > 0:
            ...
            psmStd = np.log2(df).std(axis=1)
            psmMean = np.log2(df).mean(axis=1)
...
def TMTc_summarization(inputDict, df, params, level):
    # Input arguments
    ...
    #            e.g., prot2psm: key = each protein, value = list of PSMs corresponding to the protein
    ...
    for entry, psms in inputDict.items():
        ...
        if len(psms) == 0:
            ...
            if 0 < len(subDf) <= 2:
                subDf = TMTc_summarization_1_2(subDf, params)
            elif len(subDf) >= 3:
